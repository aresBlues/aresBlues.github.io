  ...
  return (
    <div className="min-h-screen bg-[#050510] text-cyan-50 font-mono selection:bg-fuchsia-500/30 selection:text-fuchsia-200 overflow-x-hidden">
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-6 pt-32 pb-20">
        ...
        <section className="py-20 md:py-32 space-y-8 relative">
          <div className="space-y-2">
            ...
            <h1 className="text-5xl md:text-7xl font-bold tracking-tighter text-white leading-[1.1] drop-shadow-[0_0_15px_rgba(34,211,238,0.3)]">
              {site.hero.line1} <br />
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-500 via-purple-500 to-cyan-500 animate-gradient-x">{site.hero.lineAccent}</span>
              {site.hero.line2 ? (
                <>
                  <br />
                  {site.hero.line2}
          ...
          <div className="pt-8 flex gap-6">
            ...
            >
              <span className="relative z-10 flex items-center gap-2">
                {site.hero.ctaLabel} <ArrowRightIcon className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </span>
        ...
        <section id="work" className="py-20 space-y-12">
          <div className="flex items-end justify-between border-b border-cyan-900/30 pb-4">
            ...
            <span className="text-xs font-mono text-cyan-700">{site.projectsSubtitle}</span>
          </div>
              ...
              <div className="grid grid-cols-2 gap-8 font-mono text-sm">
                <div>
                  ...
                  <ul className="space-y-2 text-cyan-200/70">
                    {site.about.stackTrace.map((line) => (
                      <li key={line} className="flex items-center gap-2">
                        <span className="text-fuchsia-500">›</span> {line}
                      </li>
                    ))}
                  </ul>
                </div>
        ...
        <section id="contact" className="py-20 border-t border-cyan-900/30">
          <div className="max-w-3xl mx-auto text-center space-y-8">
            <h2 className="text-4xl md:text-5xl font-bold tracking-tight text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.5)]">
              {site.contact.headingBefore}{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-500">
                {site.contact.headingHighlight}
              </span>
            </h2>
            <p className="text-cyan-200/60 text-lg">{site.contact.subtext}</p>
            <div className="flex flex-col items-center gap-6">
              <div className="flex flex-wrap justify-center gap-4">
                ...
                <a
                  href={site.contact.repoButton.href}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="px-10 py-4 border border-cyan-500/50 text-cyan-300 font-bold tracking-widest uppercase text-sm hover:bg-cyan-950/50 transition-all hover:shadow-[0_0_20px_rgba(34,211,238,0.3)]"
                >
                  {site.contact.repoButton.label}
                </a>
              </div>
      ...
      <footer className="py-8 text-center text-xs font-mono text-cyan-900/50 border-t border-cyan-900/20">
        ...
        <p className="mt-2">
          Theme:{" "}
          <a href="https://github.com/github-samples/gitfolio" target="_blank" rel="noopener noreferrer" className="text-cyan-700 hover:text-cyan-400 transition-all">
            Gitfolio
          </a>
        </p>
      </footer>
...
function ProjectCard({
  ...
  return (
    <Link href={href} target="_blank" rel="noopener noreferrer" className={`group block space-y-4 p-6 bg-[#0a0a1a] border border-cyan-900/30 transition-all duration-300 ${colorClasses[safeColor]}`}>
